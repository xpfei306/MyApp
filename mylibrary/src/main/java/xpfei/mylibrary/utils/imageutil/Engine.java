...
class Engine {
    ...
    void setEngine(File srcImg, File tagImg) throws IOException {
        if (isJpeg(srcImg)) {
    ...
    File compress() throws IOException {
        ...
        Bitmap tagBitmap = decodeFile(srcImg.getAbsolutePath(), options);
    ...
    String compress1() throws IOException {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inSampleSize = computeSize();
        ...
        tagBitmap = rotatingImage(tagBitmap);
        tagBitmap.compress(Bitmap.CompressFormat.JPEG, 50, stream);
        String string = Base64.encodeToString(stream.toByteArray(), Base64.DEFAULT);
        tagBitmap.recycle();
