...
import xpfei.mylibrary.view.reclyview.callback.IHeaderCallBack;
...
public class XRefreshView extends LinearLayout {
    ...
    private void initWithContext(Context context, AttributeSet attrs) {
        // 根据属性设置参数
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.XRefreshView, 0, 0);
            try {
        ...
        public void run() {
            if (mScroller.computeScrollOffset()) {
                ...
                if (enableReleaseToLoadMore && mHolder.mOffsetY == 0 && mReleaseToLoadMore && mContentView != null && mContentView.isBottom()) {
                    ...
                    mContentView.startLoadMore(false);
                }
                post(this);
                if (isStopLoadMore) {
                    scrollback(offsetY);
