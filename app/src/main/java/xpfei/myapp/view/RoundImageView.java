...
public class RoundImageView extends ImageView {
    ...
    public RoundImageView(Context context, AttributeSet attrs) {
        ...
        mBorderRadius = a.getDimensionPixelSize(
                R.styleable.RoundImageView_borderRadius, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
    ...
    private void setUpShader() {
        ...
        mBitmapShader = new BitmapShader(bmp, TileMode.CLAMP, TileMode.CLAMP);// 将bmp作为着色器，就是在指定区域内绘制bmp
        float scale = 1.0f;
        if (type == TYPE_CIRCLE) {
            int bSize = Math.min(bmp.getWidth(), bmp.getHeight());  // 拿到bitmap宽或高的小值
